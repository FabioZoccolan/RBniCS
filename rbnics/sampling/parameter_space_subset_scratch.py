...
from rbnics.sampling.distributions import CompositeDistribution, UniformDistribution, DrawFrom #added DrawFrom
...
class ParameterSpaceSubset(ExportableList): # equivalent to a list of tuples
    ...
    def generate(self, box, n, sampling=None, typeGrid=0, order_flag=False): # typeGrid=0, order_flag=False):
        ...
        if len(box) > 0:
            if sampling is None:
                print("There is no Sampling ---> uniform distribution") #added
            ...
            def run_sampling(): # HERE WE GENERATE THE SAMPLE!!   
                print("\nParameterSpaceSubset.generate::now calling compositeDistribution.sample")
