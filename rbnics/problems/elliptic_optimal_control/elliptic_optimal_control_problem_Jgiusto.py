...
class EllipticOptimalControlProblem(EllipticOptimalControlProblem_Base):
    ...
    def __init__(self, V, **kwargs):
        ...
        self.terms = ["a", "a*", "c", "c*", "m", "n", "f", "g", "h", "g_o", "m_o", "n_o" ]
        self.terms_order = {
            "a": 2, "a*": 2,
            "c": 2, "c*": 2,
            "m": 2, "n": 2,
    ...
    def _compute_output(self):
        assembled_operator = dict()
        for term in ("g_o", "h", "m_o", "n_o"): #for term in ("g", "h", "m", "n"):
            assembled_operator[term] = sum(product(self.compute_theta(term), self.operator[term]))
        self._output = (
            0.5 * (transpose(self._solution) * assembled_operator["m_o"] * self._solution)
            + 0.5 * (transpose(self._solution) * assembled_operator["n_o"] * self._solution)
            - transpose(assembled_operator["g_o"]) * self._solution
