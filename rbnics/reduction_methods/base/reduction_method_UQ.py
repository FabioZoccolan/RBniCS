...
class ReductionMethod(object, metaclass=ABCMeta):
    def __init__(self, folder_prefix):
        print("ReductionMethod is initialized")
        # I/O
        ...
        print("Finished ReductionMethod__init__ \n") #added
    ...
    def initialize_training_set(self, mu_range, ntrain, enable_import=True, sampling=None, typeGrid=0, order_flag=False, **kwargs):
        print("\n **In initialize_training_set of ReductionMethod")
        # Create I/O folder
        ...
        if not import_successful:
            self.training_set.generate(mu_range, ntrain, sampling, typeGrid, order_flag)
            # Export
            print("ReductionMethod.initialize_training_set::ATTEMPTING TO WRITE WEIGHTS TO FILE")
            self.training_set.save(self.folder["training_set"], "training_set", typeGrid)
            print("ReductionMethod.initialize_training_set:: WRITTEN WEIGHTS TO FILE SUCCESSFULLY")
        print("**Finished initialize_training set of ReductionMethod\n")        
        return import_successful
        
    # ERROR ANALYSIS: set the elements in the testing set.
    def initialize_testing_set(self, mu_range, ntest, enable_import=False, sampling=None, typeGrid=0, order_flag=False, **kwargs):
        # Create I/O folder
        ...
        if not import_successful:
            self.testing_set.generate(mu_range, ntest, sampling, typeGrid=0, order_flag=False)
            # Export
