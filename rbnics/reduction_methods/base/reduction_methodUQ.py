...
class ReductionMethod(object, metaclass=ABCMeta):
    ...
    def initialize_training_set(self, mu_range, ntrain, enable_import=True, sampling=None, quadrature=0, typeGrid=0, **kwargs):
        print("*In initialize_training_set of ReductionMethod")
        # Create I/O folder
        ...
        if not import_successful:
            self.training_set.generate(mu_range, ntrain, sampling, quadrature, typeGrid)
            # Export
            #self.training_set.save(self.folder["training_set"], "training_set")
            print("ReductionMethod.initialize_training_set::ATTEMPTING TO WRITE WEIGHTS TO FILE")   #AGGIUNTO IO
            self.training_set.save(self.folder["training_set"], "training_set", typeGrid) #AGGIUNTO IO
        print("**Finished initialize_training set of ReductionMethod") 
    ...
    def initialize_testing_set(self, mu_range, ntest, enable_import=False, sampling=None, **kwargs):
        print("se sono qui reduction method initialize_testing_set, potrebbe mancare quadrature e typeGrid") 
        print("Sono in reduction_method; initialize_testing_set: FORSE MANCA QUALCOSA")
        # Create I/O folder
        ...
        if not import_successful:
            print("se sono qui reduction method testing_set, potrebbe mancare quadrature e typeGrid DOPO IF NOT IMPORT_SUCCESSFUL") 
            self.testing_set.generate(mu_range, ntest, sampling, quadrature=0, typeGrid=0)
            print("se sono qui reduction method testing_set, perchè ho messo quadrature e typegrid=0 di default") 
            # Export
