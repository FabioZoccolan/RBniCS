...
def SmolyakRule(d, q, rule, bounds, param=[], is_loguniform=False):  
    ...
    print("bounds=", bounds)
    #print("param=", param)
    print('\nrange =', range(max(d,q-d+1), q+1)) #####range non conta l'ultimo, quindi va da max(d,q-d+1) a q, entrambi compresi
    for l in range(max(d,q-d+1), q+1):  
        ...
        if l == d:
            ...
            for i in range(d):
                ...
                print("bounds[i], param[i]=", bounds[i], param[i])
                tmpnodes[i,0], tmp2 = au.univariate_rule(1, rule, bounds[i], param[i])
                tmpweights[0,0] = tmpweights[0,0]*tmp2
            ...
            #print("tmpnodes, tmpweights=", tmpnodes, tmpweights) 
            ...
            for i in range(m):
                ...
                tmpnodes, tmpweights = au.univariate_rule(int(gamma[0]), rule, bounds[0], param[0])         
                ...
                for j in range(1,d): # here the nodes and weights are defined for integration on the standard interval
                    tmp1, tmp2 = au.univariate_rule(int(gamma[j]), rule, bounds[j], param[j])
