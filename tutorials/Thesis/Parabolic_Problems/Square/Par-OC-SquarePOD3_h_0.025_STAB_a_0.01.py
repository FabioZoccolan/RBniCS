...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        self.y_0 = Expression("0.0+1.0*(x[0] == 0.0)*(x[1] <= 0.25)+1.0*(x[1]==0.0)", degree=1, domain=mesh) #self.y_0 = Expression("1.0-1.0*(x[0]==0)-1.0*( x[0] <= 1)*(x[1]==0)-1.0*(x[0] <= 1)*( x[1]==1) ", degree=1, domain=mesh)
...
# 1. Mesh
mesh = Mesh("data/squareOC_h_0.027.xml")
subdomains = MeshFunction("size_t", mesh, "data/squareOC_h_0.027_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/squareOC_h_0.027_facet_region.xml")
