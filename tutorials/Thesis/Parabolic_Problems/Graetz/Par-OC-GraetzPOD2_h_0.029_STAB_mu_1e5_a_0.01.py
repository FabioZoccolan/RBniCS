...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        self.y_0 = Expression("1.0-1.0*(x[0]==0)-1.0*( x[0] <= 1)*(x[1]==0)-1.0*(x[0] <= 1)*( x[1]==1) ", degree=1, domain=mesh)
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            for i in range(Nt):
                ...
                a1_0[i, i] =  dt*vel*y[i].dx(0)*q[i]*dx #a1 = vel * y.dx(0) * q * dx
                a2_0[i, i] =  inner(y[i],q[i])*dx 
                a3_0[i, i] =  dt * h * vel * y[i].dx(0) * q[i].dx(0) * dx(4)
                a4_0[i, i] =  dt * h * y[i] * q[i].dx(0) * dx
            for i in range(Nt-1):
                a2_0[i+1, i] = - inner(y[i],q[i+1])*dx 
            ...
            for i in range(Nt):
                ...
                as2_0[i, i] = inner(z[i],p[i])*dx 
                as3_0[i, i] = dt*h * vel * p[i].dx(0) * z[i].dx(0) * dx(1) + dt*h * vel * p[i].dx(0) * z[i].dx(0) * dx(2) + dt*h * vel * p[i].dx(0) * z[i].dx(0) * dx(3) + dt*h * vel * p[i].dx(0) * z[i].dx(0) * dx(4)
                as4_0[i,i] =   dt * h * z[i].dx(0) * p[i] * dx
            for i in range(Nt-1):
                as2_0[i+1, i] = - inner(p[i+1], z[i])*dx
            ...
            for i in range(Nt):
                ...
                cs1_0[i,i] = + dt* h * inner(p[i], v[i])*ds(1) #togliere (1) in case
            ...
            for i in range(Nt):
                ...
                g1_0[i] = - dt * h * y_d * z[i] * dx(3) + - dt* h * y_d * z[i] * dx(4)
            g0 = [g0_0, 0, 0]
            g1 = [g1_0, 0, 0]
