...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def __init__(self, block_V, **kwargs): #ho messo block_V invece che V
        ...
        self.y_0 = Expression("1.0", degree=1, domain=mesh) #-1.0*(x[0]==0)-1.0*( x[0] <= 1)*(x[1]==0)-1.0*(x[0] <= 1)*( x[1]==1) 
    ...
    def name(self):
        return "Parabolic_GraetzPOD_STAB_h_0.040_mu_1e5_alpha_0.01"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            for i in range(Nt):
                ...
                m_1[i,i] = h * inner(u[i], v[i].dx(0))*dx
                m_1[0,0] = 0.0
            for i in range(Nt-1):
                m_0[i+1,i] = - inner(u[i], v[i+1])*dx
                m_1[i+1,i] = -h * inner(u[i], v[i+1].dx(0)) *dx #MANCA UN TERMINE
...
# 1. Mesh
mesh = Mesh("data/graetz_h_0.040.xml")
subdomains = MeshFunction("size_t", mesh, "data/graetz_h_0.040_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/graetz_h_0.040_facet_region.xml")
...
problem.solve()
problem.export_solution(filename="FEM_Par_Graetz_STAB_h_0.040_mu_1e5_alpha_0.01")
...
reduced_problem.export_solution(filename="online_solution_Par_Graetz_STAB_h_0.040_with_stabilization_mu_10.0")
reduced_problem.solve(online_stabilization=False)
reduced_problem.export_solution(filename="online_solution_Par_Graetz_STAB_h_0.040_without_stabilization_mu_10.0")

# ### 4.7. Perform an error analysis

# 7. Perform an error analysis
reduction_method.initialize_testing_set(100)
reduction_method.error_analysis(online_stabilization=True, filename="error_analysis_Par_Graetz_STAB_h_0.040_with_stabilization_mu_10.0")
reduction_method.error_analysis(online_stabilization=False, filename="error_analysis_Graetz_STAB_h_0.040_without_stabilization_mu_10.0")

# 8. Perform a speedup analysis
reduction_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_Par_Graetz_STAB_h_0.040_with_stabilization_mu_10.0")
