...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def name(self):
        return "Parabolic_OCGraetzPOD1_GEOM_h_0.020_STAB_mu_1e4.8_3.3_alpha_0.01_d_1"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            for i in range(Nt-1):
                ...
                m_1[i+1, i] = - h * inner(y[i], q[i+1].dx(0))*dx(1)
                m_2[i+1, i] = - h * inner(y[i], q[i+1].dx(0))*dx(2)
            ...
            for i in range(Nt):
                ...
                as2_0[i, i] = - dt*h * vel * p[i].dx(0) * z[i].dx(0) * dx(2)
                as3_0[i, i] = - dt * p[i].dx(1) * z[i].dx(1) * dx(2) 
                as4_0[i, i] = - dt*vel*p[i].dx(0)*z[i]*dx(2) 
                ...
                ms_1[i, i] = - h * inner(p[i].dx(0), z[i])*dx(1)
                ms_2[i, i] = - h * inner(p[i].dx(0), z[i])*dx(2)
            for i in range(Nt-1):
                ms_0[i+1, i] = - inner(p[i+1], z[i])*dx
                ms_1[i+1, i] = + h*inner(p[i+1].dx(0), z[i])*dx(1)
                ms_2[i+1, i] = + h*inner(p[i+1].dx(0), z[i])*dx(2)
            ...
            for i in range(Nt):
                m0_0[i,i] = dt*inner(y[i], z[i])*dx(1)
                m1_0[i,i] = dt*inner(y[i], z[i])*dx(2) + dt*inner(y[i], z[i])*dx(3) + dt*inner(y[i], z[i])*dx(4)
                m2_0[i,i] = - dt * h * inner(y[i], z[i].dx(0)) * dx(1)
                m3_0[i,i] = - dt * h * inner(y[i], z[i].dx(0)) * dx(2) - dt * h * inner(y[i], z[i].dx(0)) * dx(3) - dt * h * inner(y[i], z[i].dx(0)) * dx(4) 
            m0 = [[m0_0, 0, 0], [0, 0, 0], [0, 0, 0]]
            m1 = [[m1_0, 0, 0], [0, 0, 0], [0, 0, 0]]
            m2 = [[m2_0, 0, 0], [0, 0, 0], [0, 0, 0]]
            m3 = [[m3_0, 0, 0], [0, 0, 0], [0, 0, 0]]
            return (BlockForm(m0),BlockForm(m1),BlockForm(m2),BlockForm(m3))
        elif term == "n":
            u = self.u
            v = self.v
