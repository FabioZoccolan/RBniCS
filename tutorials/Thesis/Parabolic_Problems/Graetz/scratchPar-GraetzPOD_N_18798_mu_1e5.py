...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def name(self):
        return "scratchParabolic_OCGraetzRB_N_18798_mu_1e5_alpha_0.01"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            bc0 = [ BlockDirichletBC([[DirichletBC(block_V.sub(i), Constant(0.0), self.boundaries, 1) for i in range(0, Nt)]]),
                   BlockDirichletBC([[DirichletBC(block_V.sub(i), Constant(1.0), self.boundaries, 2) for i in range(0, Nt)]]),
                   BlockDirichletBC([[DirichletBC(block_V.sub(i), Constant(1.0), self.boundaries, 4) for i in range(0, Nt)]]),
                   BlockDirichletBC([[DirichletBC(block_V.sub(i), Constant(0.0), self.boundaries, 5) for i in range(0, Nt)]]),
                   BlockDirichletBC([[DirichletBC(block_V.sub(i), Constant(0.0), self.boundaries, 6) for i in range(0, Nt)]])]
            return (bc0,)
...
problem.solve()
problem.export_solution(filename="FEM_Par_OCGraetz_N_18798_mu_1e5_alpha_0.01")
...
reduced_problem.export_solution(filename="online_solution_Par_Graetz_NOSTAB_N_5503_with_stabilization_mu_10.0")
reduced_problem.solve(online_stabilization=False)
reduced_problem.export_solution(filename="online_solution_Par_Graetz_NOSTAB_N_5503_without_stabilization_mu_10.0")

# ### 4.7. Perform an error analysis

# 7. Perform an error analysis
reduction_method.initialize_testing_set(100)
reduction_method.error_analysis(online_stabilization=True, filename="error_analysis_Par_Graetz_NOSTAB_N_5503_with_stabilization_mu_10.0")
reduction_method.error_analysis(online_stabilization=False, filename="error_analysis_Graetz_NOSTAB_N_5503_without_stabilization_mu_10.0")

# 8. Perform a speedup analysis
reduction_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_Par_Graetz_NOSTAB_N_5503_with_stabilization_mu_10.0")
reduction_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_Par_Graetz_NOSTAB_N_5_without_stabilization_mu_10.0") 
