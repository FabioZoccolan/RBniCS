...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def __init__(self, V, **kwargs):
        ...
        self.delta = 2.1 # delta_k on the notes of Torlo
        self.h = 1.0 #CellDiameter(V.mesh())

    # Return custom problem name
    def name(self):
        return "scratchAdvectionDominated2POD"
    ...
    def compute_theta(self, term):
        ...
        if term == "a":
            ...
            if self.stabilized:
                delta = self.delta
                theta_a3 = 0.0 #delta * cos(mu[1])
                theta_a4 = 0.0 # delta * sin(mu[1])
            ...
            theta_f0 = - 1/(mu[0]) - cos(mu[1]) - sin(mu[1])
            if self.stabilized:
                delta = self.delta
                theta_f1 = -delta * ( cos(mu[1])**2+cos(mu[1]) * sin(mu[1]) )
                theta_f2 = -delta * ( sin(mu[1])**2+cos(mu[1]) * sin(mu[1]) )
