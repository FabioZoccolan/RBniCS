...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def name(self):
        return "scratchAdvectionOCGraetzPOD_N_18798_STAB_mu_1e5_alpha_0.01"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            h = self.h
            cs0 = v * p * dx
            cs1 = - h * v.dx(0) * p * dx # Constant(0.0) 
            return (cs0,cs1)
        elif term == "m":
            y = self.y
            z = self.z
            ...
            h = self.h
            n0 = u * v * dx
            n1 = -h * u * v.dx(0) * dx # Constant(0.0)*-h * u * v.dx(0) * dx
            return (n0,n1)
        elif term == "f":
            q = self.q
...
mesh = Mesh("data/graetzOC_Z_.xml")
subdomains = MeshFunction("size_t", mesh, "data/graetzOC_Z__physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/graetzOC_Z_facet_region.xml")
...
problem.solve()
problem.export_solution(filename="scratch_FEM_OCGraetz_N_18798_STAB_mu_1e5_alpha_0.01")
...
reduced_elliptic_optimal_control.export_solution(filename="scratch_online_solution_OCGraetz_N_18798_STAB_mu_1e5_alpha_0.01")
