...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def name(self):
        return "Numerical_Results/viscosity"
...
offline_mu = (1e5,) 
problem.init() #####CAMBIAREEE METTERE DOPO
    ...
    for online_vanishing_viscosity  in (True, False):
        def filename(prefix):
            return ("online_" + prefix + "_" + bool_to_string(online_stabilization) + "_stabilization_and_"
                    + bool_to_string(online_vanishing_viscosity ) + "_rectification")
        reduced_elliptic_optimal_control.solve(
            online_stabilization=online_stabilization, online_vanishing_viscosity =online_vanishing_viscosity )
        reduced_elliptic_optimal_control.export_solution(filename=filename("solution"))
        reduced_elliptic_optimal_control.export_error(filename=filename("error"))
...
for online_stabilization in (True, False):
    for online_vanishing_viscosity  in (True, False):
        ...
        pod_galerkin_method.error_analysis(
            online_stabilization=online_stabilization, online_vanishing_viscosity =online_vanishing_viscosity , filename=filename)
...
pod_galerkin_method.error_analysis(online_stabilization=False, filename="error_analysis_OCGraetz2_h_0.029_OffSTAB_mu_1e5_alpha_0.01")

print("\n--------------------------------------ONLINE-OFFLINE STABILIZATION ERROR ANALYSIS BEGINS--------------------------------------------\n")

pod_galerkin_method.error_analysis(online_stabilization=True, filename="error_analysis_OCGraetz2_h_0.029_OffONSTAB_mu_1e5_alpha_0.01")
...
print("")
pod_galerkin_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_OCGraetz2_h_0.029_OffSTAB_mu_1e5_alpha_0.01")
print("\n---------------------------------------ONLINE-OFFLINE STABILIZATION SPEED-UP ANALYSIS BEGINS------------------------------------------\n")
pod_galerkin_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_OCGraetz2_h_0.029_OffONSTAB_mu_1e5_alpha_0.01")
'''
