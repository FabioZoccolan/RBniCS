...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def __init__(self, V, **kwargs):
        ...
        self.lifting = Expression("((x[0] >= 0 && x[1] == 0) || (x[1] <= 0.25 && x[0] == 0) )? 1. : 0.", degree = self.V.ufl_element().degree(), domain=mesh) #degree=1
    ...
    def name(self):
        return "AdvectionSquareRB_N_15676_NOTSTAB_mu_2e4_0.8"
...
mesh = Mesh("data/square_N_15676.xml")
subdomains = MeshFunction("size_t", mesh, "data/square_N_15676_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/square_N_15676_facet_region.xml")
...
problem.solve()
problem.export_solution(filename="FEM_adsquare_NOSTAB_N_15676_mu_2e4_0.8") #_with_stabilization
...
reduced_problem.export_solution(filename="online_solution_adsquare_NOSTAB_N_15676_with_stabilization_mu_2e4_0.8") 

reduced_problem.solve(online_stabilization=False)
reduced_problem.export_solution(filename="online_solution_adsquare_NOSTAB_N_15676_without_stabilization_mu_2e4_0.8")
# 7. Perform an error analysis
reduction_method.initialize_testing_set(100)
reduction_method.error_analysis(online_stabilization=True, filename="error_analysis_adsquare_NOSTAB_N_15676_with_stabilization_mu_2e4_0.8")
reduction_method.error_analysis(online_stabilization=False, filename="error_analysis_adsquare_NOSTAB_N_15676_without_stabilization_mu_2e4_0.8")

# 8. Perform a speedup analysis
reduction_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_adsquare_NOSTAB_N_15676_with_stabilization_mu_2e4_0.8")
reduction_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_adsquare_NOSTAB_N_15676_without_stabilization_mu_2e4_0.8")
