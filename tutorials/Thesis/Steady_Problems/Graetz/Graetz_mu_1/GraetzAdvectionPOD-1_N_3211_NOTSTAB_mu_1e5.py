...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def name(self):
        return "AdvectionRBGraetz-1_N_3211_NOTSTAB_mu_1e5"
    ...
    def compute_theta(self, term):
        ...
        if term == "a":
            ...
            if self.stabilized:
                delta = self.delta
                theta_a2 = 0.0 #delta * 4.0
                theta_a3 = 0.0 #delta * 4.0
            else:
                theta_a2 = 0.0
                theta_a3 = 0.0
...
mesh = Mesh("data/graetz_N_3211.xml")
print("hmax", mesh.hmax())
subdomains = MeshFunction("size_t", mesh, "data/graetz_N_3211_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/graetz_N_3211_facet_region.xml")
...
problem.solve()
problem.export_solution(filename="FEM_Graetz_NOSTAB_N_3211_offline_mu_1e5")
...
reduced_problem.export_solution(filename="online_solution_Graetz_NOSTAB_N_3211_with_stabilization_mu_1e5")
reduced_problem.solve(online_stabilization=False)
reduced_problem.export_solution(filename="online_solution_Graetz_NOSTAB_N_3211_without_stabilization_mu_1e5")

# 7. Perform an error analysis
reduction_method.initialize_testing_set(100)
reduction_method.error_analysis(online_stabilization=True, filename="error_analysis_Graetz_NOSTAB_N_3211_with_stabilization_mu_1e5")
reduction_method.error_analysis(online_stabilization=False, filename="error_analysis_Graetz_NOSTAB_N_3211_without_stabilization_mu_1e5")

# 8. Perform a speedup analysis
reduction_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_Graetz_NOSTAB_N_3211_with_stabilization_mu_1e5")
reduction_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_Graetz_NOSTAB_N_3211_without_stabilization_mu_1e5") 
