...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def name(self):
        return "AdvectionOCGraetzRB2ExSTAB"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            f1 = h * vel * l.dx(0) * v.dx(0) * dx #(1)
            f2 = h * vel * l.dx(0) * v.dx(0) * dx #(2) 
            return (f0, f1, f2)
        elif term == "g":
            y_d = self.y_d
            z = self.z
...
reduced_solution_ns = reduced_problem.solve(online_stabilization=False)
