...
class AdvectionDominated(EllipticCoerciveProblem):
    ...
    def __init__(self, V, **kwargs):
        ...
        self.lifting = Expression('((x[0] >= 0 && x[1] == 0) || (x[1] <= 0.25 && x[0] == 0) ) ? 1. : 0.', degree=1, domain=mesh)
    ...
    def compute_theta(self, term):
        ...
        if term == "a":
            ...
            if self.stabilized:
                delta = self.delta
                theta_f3 = - delta * cos(mu[1])**2
                theta_f4 = - delta * cos(mu[1]) * sin(mu[1])
                theta_f5 = - delta * sin(mu[1])**2
            else:
                theta_f3 = 0.0
                theta_f4 = 0.0
                theta_f5 = 0.0
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            u = self.u
            h = self.h
            a0 = inner(grad(u), grad(v)) * dx #- inner(grad(l), grad(v)) * dx
            a1 = u.dx(0) * v * dx #- l.dx(0) * v * dx
            a2 = u.dx(1) * v * dx #- l.dx(1) * v * dx
            a3 = h * u.dx(0) * v.dx(0) * dx #- h * l.dx(0) * v.dx(0) * dx
            a4 = h * u.dx(0) * v.dx(1) * dx + h * u.dx(1) * v.dx(0) * dx #- h * l.dx(0) * v.dx(1) * dx + h * l.dx(1) * v.dx(0) * dx
            a5 = h * u.dx(1) * v.dx(1) * dx# - h * l.dx(1) * v.dx(1) * dx 
...
problem.solve()
problem.export_solution(filename="offline_solution_with_stabilization")   #in caso mettere prima che mi costruisca la base ridotta
