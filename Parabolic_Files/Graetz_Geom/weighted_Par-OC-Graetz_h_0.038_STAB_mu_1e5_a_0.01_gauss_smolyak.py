...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        self.y_0 = Expression("0.0", degree=1, domain=mesh)  #Expression("1.0", degree=1, domain=mesh) 
       ...
       # Customize linear solver parameters
        ...
        PETScOptions.set("mat_mumps_icntl_14", 99)

    # Return custom problem name
    def name(self):
        return "weighted_Parabolic_OCGraetzPOD3_GEOM_h_0.043_STAB_mu_1e5_0.5_a_0.01_T_3_t_0.1_beta0503_0503_gauss_smolyak"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            for i in range(Nt):
                ...
                a6_0[i, i] =  dt* h * vel * y[i].dx(0) * q[i].dx(0) * dx(2) + dt*h * vel * y[i].dx(0) * q[i].dx(0) * dx(3) + dt*h * vel * y[i].dx(0) * q[i].dx(0) * dx(4)            
                ...
                m_3[i, i] =  h * inner(y[i], q[i].dx(0))*dx(2) + h * inner(y[i], q[i].dx(0))*dx(3) + h * inner(y[i], q[i].dx(0))*dx(4)
            for i in range(Nt-1):
                m_0[i+1, i] = - inner(y[i],q[i+1])*dx(1)
                m_1[i+1, i] = - inner(y[i],q[i+1])*dx(2) - inner(y[i],q[i+1])*dx(3) - inner(y[i],q[i+1])*dx(4)
                m_2[i+1, i] = - h * inner(y[i], q[i+1].dx(0))*dx(1)
                m_3[i+1, i] = - h * inner(y[i], q[i+1].dx(0))*dx(2) - h * inner(y[i], q[i+1].dx(0))*dx(3) - h * inner(y[i], q[i+1].dx(0))*dx(4)
            ...
            for i in range(Nt):
                ...
                as2_0[i, i] = - dt* p[i].dx(0) * z[i].dx(0) * dx(2) - dt * p[i].dx(0) * z[i].dx(0) * dx(3) - dt * p[i].dx(0) * z[i].dx(0) * dx(4) 
                ...
                as6_0[i, i] =  dt * h * vel * p[i].dx(0) * z[i].dx(0) * dx(2) +dt*h * vel * z[i].dx(0) * p[i].dx(0) * dx(3) + dt*h * vel * z[i].dx(0) * p[i].dx(0) * dx(4)  
                ms_0[i, i] = + inner(z[i],p[i])*dx(1)
                ms_1[i, i] = + inner(z[i],p[i])*dx(2)  + inner(z[i],p[i])*dx(3)  + inner(z[i],p[i])*dx(4)
                ms_2[i, i] = - h * inner(p[i], z[i].dx(0))*dx(1)
                ms_3[i, i] = - h * inner(p[i], z[i].dx(0))*dx(2) - h * inner(p[i], z[i].dx(0))*dx(3) - h * inner(p[i], z[i].dx(0))*dx(4)
            for i in range(Nt-1):
                ms_0[i, i+1] = - inner(p[i+1], z[i])*dx(1)
                ms_1[i, i+1] = - inner(p[i+1], z[i])*dx(2) - inner(p[i+1], z[i])*dx(3) - inner(p[i+1], z[i])*dx(4)
                ms_2[i, i+1] = + h*inner(p[i+1], z[i].dx(0))*dx(1)
                ms_3[i, i+1] = + h*inner(p[i+1], z[i].dx(0))*dx(2) + h*inner(p[i+1], z[i].dx(0))*dx(3) + h*inner(p[i+1], z[i].dx(0))*dx(4)
...
elliptic_optimal_control.export_solution(filename="FEM_UQ_beta0503_0503_Par_OCGraetz_h_0.038_STAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
...
pod_galerkin_method.initialize_training_set(91, sampling=BetaDistribution(beta_a, beta_b), typeGrid=2)
...
#reduced_elliptic_optimal_control.export_solution(filename="online_solution_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
#reduced_elliptic_optimal_control.export_error(filename="online_error_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
...
reduced_elliptic_optimal_control.export_solution(filename="online_solution_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffONSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
reduced_elliptic_optimal_control.export_error(filename="online_error_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffONSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
...
pod_galerkin_method.error_analysis(online_stabilization=False, filename="error_analysis_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")

print("\n--------------------------------------ONLINE-OFFLINE STABILIZATION ERROR ANALYSIS BEGINS--------------------------------------------\n")

pod_galerkin_method.error_analysis(online_stabilization=True, filename="error_analysis_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffONSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
...
#print("")
#pod_galerkin_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
print("\n---------------------------------------ONLINE-OFFLINE STABILIZATION SPEED-UP ANALYSIS BEGINS------------------------------------------\n")
pod_galerkin_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_UQ_beta0503_0503_Par_OCGraetz_h_0.038_OffONSTAB_mu_1e5_alpha_0.01_gauss_smolyak_article2")
