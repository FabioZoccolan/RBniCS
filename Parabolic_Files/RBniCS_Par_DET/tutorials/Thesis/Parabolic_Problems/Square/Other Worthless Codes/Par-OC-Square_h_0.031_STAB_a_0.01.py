...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        self.y_0 = Expression("0.0 + 1.0*(x[0] == 0.0)*(x[1] == 0.25)", degree=1, domain= mesh) #self.y_0 = Expression("1.0-1.0*(x[0]==0)-1.0*( x[0] <= 1)*(x[1]==0)-1.0*(x[0] <= 1)*( x[1]==1) ", degree=1, domain=mesh)
    ...
    def name(self):
        return "Parabolic_OCSquare_h_0.031_STAB_mu_2e4_1.2_alpha_0.01_d_1.0"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            for i in range(Nt-1):
                ...
                ms_2[i+1, i] = + h * inner(p[i+1], z[i].dx(1))*dx
...
# 1. Mesh
mesh = Mesh("data/squareOC_h_0.031.xml")
subdomains = MeshFunction("size_t", mesh, "data/squareOC_h_0.031_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/squareOC_h_0.031_facet_region.xml")
...
elliptic_optimal_control.export_solution(filename="FEM_Par_OCSquare_h_0.031_STAB_mu_2e4_1.2_alpha_0.01_d_1.0")
...
reduced_elliptic_optimal_control.export_solution(filename="online_solution_Par_OCSquare_h_0.031_OffSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
reduced_elliptic_optimal_control.export_error(filename="online_error_Par_OCSquare_h_0.031_OffSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
...
reduced_elliptic_optimal_control.export_solution(filename="online_solution_Par_OCSquare_h_0.031_OffONSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
reduced_elliptic_optimal_control.export_error(filename="online_error_Par_OCSquare_h_0.031_OffONSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
...
pod_galerkin_method.error_analysis(online_stabilization=False, filename="error_analysis_Par_OCSquare_h_0.031_OffSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")

print("\n--------------------------------------ONLINE-OFFLINE STABILIZATION ERROR ANALYSIS BEGINS--------------------------------------------\n")

pod_galerkin_method.error_analysis(online_stabilization=True, filename="error_analysis_Par_OCSquare_h_0.031_OffONSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
...
print("")
pod_galerkin_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_Par_OCSquare_h_0.031_OffSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
print("\n---------------------------------------ONLINE-OFFLINE STABILIZATION SPEED-UP ANALYSIS BEGINS------------------------------------------\n")
pod_galerkin_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_Par_OCSquare_h_0.031_OffONSTAB_mu_2e4_1.2_alpha_0.01_d_1.0")
