...
class Up_Down_Control(SubDomain):
    ...
    def inside(self, x, on_boundary):
        return on_boundary and  ( (abs(x[1] - 1.) < DOLFIN_EPS and (x[0] >= self.x_min and x[0] <= self.x_max) ) or ( abs(x[1] - 0.) < DOLFIN_EPS and (x[0] >= self.x_min and x[0] <= self.x_max ) )  )
...
up_down_control = Up_Down_Control(1., 2.)
...
# Save
File("graetz_BC.xml") << mesh
File("graetz_BC_physical_region.xml") << subdomains
File("graetz_BC_facet_region.xml") << boundaries
File("graetz_BC_restriction_control.rtc.xml") << control_restriction
XDMFFile("graetz_BC.xdmf").write(mesh)
XDMFFile("graetz_BC_physical_region.xdmf").write(subdomains)
XDMFFile("graetz_BC_facet_region.xdmf").write(boundaries)
XDMFFile("graetz_BC_restriction_control.rtc.xdmf").write(control_restriction)
