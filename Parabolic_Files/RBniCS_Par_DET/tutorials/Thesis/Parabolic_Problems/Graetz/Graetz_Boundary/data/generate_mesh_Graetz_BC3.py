...
subdomain2 = Rectangle(Point(1., 0.1), Point(1.9, 0.9))
subdomain3 = Rectangle(Point(1., 0.), Point(1.9, 0.1))
subdomain4 = Rectangle(Point(1.9, 0.), Point(2., 1.))
subdomain5 = Rectangle(Point(1., 0.9), Point(1.9, 1.))
...
domain.set_subdomain(5, subdomain5) 
...
class Up_Down_Right_Control(SubDomain):
    ...
    def inside(self, x, on_boundary):
        return on_boundary and  (( (abs(x[1] - 1.) < DOLFIN_EPS and (x[0] >= self.x_min and x[0] <= self.x_max) ) or ( abs(x[1] - 0.) < DOLFIN_EPS and (x[0] >= self.x_min and x[0] <= self.x_max ) )  ) or  (abs(x[0] - 2.) < DOLFIN_EPS ) )
...
up_down_control = Up_Down_Right_Control(1., 2.)
up_down_control.mark(boundaries, 2)


topLeft = Top(0., 1.)
topLeft.mark(boundaries, 3)
...
# Save
File("graetz_BC3.xml") << mesh
File("graetz_BC3_physical_region.xml") << subdomains
File("graetz_BC3_facet_region.xml") << boundaries
File("graetz_BC3_restriction_control.rtc.xml") << control_restriction
XDMFFile("graetz_BC3.xdmf").write(mesh)
XDMFFile("graetz_BC3_physical_region.xdmf").write(subdomains)
XDMFFile("graetz_BC3_facet_region.xdmf").write(boundaries)
XDMFFile("graetz_BC3_restriction_control.rtc.xdmf").write(control_restriction)
