...
class EllipticOptimalControl(EllipticOptimalControlProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        #self.y_0 = Expression("( ( x[1] == 1 || x[1]==0)*(x[0] < 1) || ( x[0]== 0 ) )? 1.0 : 0.0", degree=1, domain=mesh)
    ...
    def name(self):
        return "Numerical_Results/Parabolic_OCGraetzPOD4_h_0.048_STAB_mu_1e5_alpha_0.01_d_1"
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            bc0 = BlockDirichletBC([[[DirichletBC(self.V.sub(i), Constant(1.0), self.boundaries, 1),
                                      DirichletBC(self.V.sub(i), Constant(1.0), self.boundaries, 5),
                                      DirichletBC(self.V.sub(i), Constant(1.0), self.boundaries, 6)] for i in range(0, Nt)], None, None])
            return (bc0,)
...
# Mesh
mesh = Mesh("data/graetzOC_h_0.048.xml")
subdomains = MeshFunction("size_t", mesh, "data/graetzOC_h_0.048_physical_region.xml")
boundaries = MeshFunction("size_t", mesh, "data/graetzOC_h_0.048_facet_region.xml")
...
elliptic_optimal_control.export_solution(filename="FEM_Par_OCGraetz4_h_0.048_STAB_mu_1e5_alpha_0.01")
...
reduced_elliptic_optimal_control.export_solution(filename="online_solution_Par_OCGraetz4_h_0.048_OffSTAB_mu_1e5_alpha_0.01")
reduced_elliptic_optimal_control.export_error(filename="online_error_Par_OCGraetz4_h_0.048_OffSTAB_mu_1e5_alpha_0.01")
...
reduced_elliptic_optimal_control.export_solution(filename="online_solution_Par_OCGraetz4_h_0.048_OffONSTAB_mu_1e5_alpha_0.01")
reduced_elliptic_optimal_control.export_error(filename="online_error_Par_OCGraetz4_h_0.048_OffONSTAB_mu_1e5_alpha_0.01")
...
pod_galerkin_method.error_analysis(online_stabilization=False, filename="error_analysis_Par_OCGraetz4_h_0.048_OffSTAB_mu_1e5_alpha_0.01")

print("\n--------------------------------------ONLINE-OFFLINE STABILIZATION ERROR ANALYSIS BEGINS--------------------------------------------\n")

pod_galerkin_method.error_analysis(online_stabilization=True, filename="error_analysis_Par_OCGraetz4_h_0.048_OffONSTAB_mu_1e5_alpha_0.01")
...
print("")
pod_galerkin_method.speedup_analysis(online_stabilization=False, filename="speedup_analysis_Par_OCGraetz4_h_0.048_OffSTAB_mu_1e5_alpha_0.01")
print("\n---------------------------------------ONLINE-OFFLINE STABILIZATION SPEED-UP ANALYSIS BEGINS------------------------------------------\n")
pod_galerkin_method.speedup_analysis(online_stabilization=True, filename="speedup_analysis_Par_OCGraetz4_h_0.048_OffONSTAB_mu_1e5_alpha_0.01")
